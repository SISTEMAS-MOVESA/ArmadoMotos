...
Partial Class OrdenProduccion
    ...
    Private Sub OrdenProduccion_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
        ...
        Try
            Dim strSQL = "select (select id from OTHEADER with(nolock) where RUTA=t.RUTA and CAMION=t.CAMION)[Id],t.CAMION, T.RUTA from  " &
                                                " (SELECT distinct ROWID [CAMION],RUTA " &
                                                    ...
                                                    " FROM [dbo].[PLAN_ARMADO] With(nolock) where [APROBADOL]='Y' AND APROBADOC='N' ) as t " &
    ...
    Private Sub repeater1_ItemDataBound(sender As Object, e As RepeaterItemEventArgs) Handles repeater1.ItemDataBound
        Try
            ...
            Dim PKId As Integer = MyTable.Rows(RepRow).Item("ID")
            Dim gvOrders As GridView = e.Item.FindControl("gvOrders")
            gvOrders.DataSource = GetData(String.Format("SELECT [ID],[HID],[CALMACEN],[NALMACEN],[MODELO],[CODIGO],[DESCRIPCION],[CANTIDAD],[ESPACIOS],[CANCELADO],[ESTADO],[USUARIO],[Preference] " &
                                                        " FROM [ArmadoMotos].[dbo].[OTLINES] where [HID]='{0}'", PKId))
            gvOrders.DataBind()
