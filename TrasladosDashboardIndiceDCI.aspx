<%@ Page Language="VB" AutoEventWireup="false" CodeFile="TrasladosDashboardIndiceDCI.aspx.vb" Inherits="TrasladosDashboardIndiceDCI" %>
    ...
    <form id="form1" runat="server">
            ...
            <div class="container-fluid">
                <div class="row my-3">
                    <div class="col-2">
                        ...
                        <button type="button" class="btn btn-success" onclick="abrirModal()">Crear Planificación</button>
                            ...
                            function abrirModal() {
                                Swal.fire({
                                    ...
                                    preConfirm: () => {
                                        ...
                                        // Obtener todas las filas del GridView (sin filtros por checkbox)
                                        let detalles = [];
                                        $("table tbody tr").each(function () {
                                            let filaData = {};
                                            $(this).find("td").each(function (index) {
                                                let header = $("table thead th").eq(index).text();
                                                filaData[header] = $(this).text().trim();
                                            });
                                            detalles.push(filaData);
                                        });

                                        if (detalles.length === 0) {
                                            Swal.showValidationMessage('No hay filas en el GridView.');
                                            return false;
                                        }

                                        return { fechaInicio, fechaVencimiento, detalles };
                                    }
                                }).then((result) => {
                                    if (result.isConfirmed) {
                                        guardarPlanificacion(result.value);
                                    }
                                });
                            }
                        ...
                        <%--<asp:Button ID="btnExportarJson" runat="server" CssClass="btn btn-block btn-secondary" Text="Planificar Seleccion" />--%>
                    </div>
                </div>
                <asp:GridView ID="gridIndiceD" runat="server" AutoGenerateColumns="false" CssClass="table table-bordered table-hover" Width="100%">
                    ...
                    <Columns>
                        ...
                        <asp:BoundField DataField="Cardcode" HeaderText="Cliente" />
                        <asp:BoundField DataField="Zona" HeaderText="Zona" />
                        <asp:BoundField DataField="Sup" HeaderText="Supervisor" />
                        <asp:BoundField DataField="Ruta" HeaderText="Ruta" />
                        <asp:BoundField DataField="Codigo" HeaderText="Almacen" />
                        <asp:BoundField DataField="Almacen" HeaderText="Nombre Almacen" />
                        <asp:BoundField DataField="70" HeaderText="Indice" ItemStyle-CssClass="heatmap-cell1" />
                        <asp:BoundField DataField="70" HeaderText="Indice" />
                        <asp:BoundField DataField="70" HeaderText="Indice" />
                        <asp:BoundField DataField="Indice_Proyectado" HeaderText="Indice Proyectado" ItemStyle-CssClass="heatmap-cell4" />
                        ...
                        <asp:TemplateField HeaderText="Codigo Almacen">
                            <ItemTemplate>
                                <div style="display: flex; align-items: center; justify-content: center; align-items: center;" class="itm-number-input">
                                    <asp:Button ID="btnSubtract"
                                        runat="server"
                                        CssClass="btn btn-info btn-sm mr-2 btnSubtract"
                                        Text='<%# Eval("Codigo") %>'
        ...
        <script>
            ...
            var colorScale4 = d3.scaleSequential()
                ...
                .interpolator(d3.interpolateOranges); // Gradiente de amarillo a rojo (d4-color-scheme)
